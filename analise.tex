...
\begin{definicao}[ Não Pertinência]
	...
	$$x \not \in A \iff \neg (x\in A)$$
\end{definicao}
\begin{axioma}[Axioma da Base]
	Todo elemento matemático é um conjunto e dados conjuntos \(A,B\), temos \(A\in B \) ou \( A\not \in B\). 
\end{axioma}
...
\end{proof}
\subsection{Axioma do Pareamento}
\begin{axioma}[Axioma da existência]
	\[ \exists A \: : \forall x \:  ( x\not \in A ) \]
\end{axioma}
\begin{prop}
	Existe um único conjunto conjunto \(A\) tal que \(\forall x\: (x\not \in A) \).
\end{prop}
\begin{proof}
	Sejam \(A,B\) com  \(\forall x\: ( x\not \in A) \) e \(\forall x\: ( x\not \in B) \). Se \(A\neq B\), deve existir \(x\in A\) com \(x\not \in B\), ou \(x\in B \) com \(x\not \in A\), mas não existe \(x\) com \(x\in A\) ou \(x\in B\), logo \(A = B\). 
\end{proof}
\begin{definicao}
	O conjunto conjunto \(A\) tal que \(\forall x\: (x\not \in A) \) será chamado de vazio, e usaremos o símbolo \(A = \emptyset\).
\end{definicao}
\begin{prop}
	Para todo \(A\), temos \(\emptyset \subset A\)
\end{prop}
\begin{proof}
	Dado \(A\), para \(\emptyset\not \subset A\), deveria existir um elemento \(x\in \emptyset\) com \(x\not \in A\), o que não é possível, logo \(\emptyset \subset A\).
\end{proof}
\begin{axioma}[Axioma do Par]
	Para quaisquer dois conjuntos \(A,B\), existe um conjunto \(C\) tal que \(A\in C \) e \(B\in C\).
	\[\forall A\: : \: \forall B \: : \: \exists C \: : \: ( A\in C\land B \in C  ) \]
\end{axioma}
\begin{prop}
	Existe um conjunto \(C\) tal que \( A \in C \) e \( B\in C\) se, e somente se, existe um conjunto \(C\) tal que \(D \in C \iff D = A\lor D = B \) (\( A,B\) são os únicos  elementos de \(C\) ).
\end{prop}
\begin{proof}
	Se existe um conjunto \(C\) tal que \( D\in C \iff D = A \lor D = B\), então existe um conjunto \(C\) tal que \(A\in C\) e \(B\in C\). 

	Supondo que existe um conjunto \(C\) tal que \( A\in C\) e \( B \in C\). Pelo axioma da especificação, podemos tomar \( C' = \{ D\in C \: | \: D=  A \lor D  = B\} \) e o resultado está provado.
\end{proof}
\begin{definicao}
	Dados \(A,B\) quaisquer, o conjunto \(C\) tal que \(D\in C \iff D = A \lor D = B\) existe e é único pelo axioma da extensão. O denotaremos por \[ \{ A,B\}. \]
	Tal conjunto é um par.
\end{definicao}
\begin{definicao}
	Dado um conjunto \(A\) qualquer, podemos fomar o par \( \{ A,A\} \). Esse par é denotado por \[ \{ A \} = \{ A, A\} \]
	e chama-se "singleton" de \(a\). Ele é caracterizado pelo fato de \( D\in \{ A\} \iff D = A\).
\end{definicao}
\subsection{Uniões e Interseções}
\subsubsection{União}

\begin{axioma}[Axioma da União]
	Para todo conjunto \( A\), existe um conjunto \(U\) tal que \( \forall D ( \exists B (D\in B \land B\in A) \implies D \in U) \).
	\[ \forall A \: : \:  \exists U  \: : \: \forall D  \: : \: (  ( \exists B \: : \:  (D \in B \land B\in A) )  \implies D\in U ) \]
\end{axioma}
\begin{prop}
	Dado \( A\), existe um conjunto \(U\) tal que \( \forall D ( \exists B (D\in B \land B\in A) \implies D \in U) \) se, e somente se, existe um conjunto \(U\) tal que \( \forall D ( \exists (D\in B \land B\in A) \iff D \in U) \).
\end{prop}
\begin{proof}
Se existe um conjunto \(U\) tal que \( \forall D ( (D\in B \land B\in A) \iff D \in U) \), então existe um conjunto \(U\) tal que \( \forall D ( \exists B (D\in B \land B\in A) \implies D \in U) \).
	Se existe um conjunto \(U\) tal que \( \forall D ( \exists B (D\in B \land B\in A) \implies D \in U) \), então tomando \( U' = \{ D\in U \: | \:  \exists B : ( D\in B \land D \in A ) \} \) conclui a demonstração.
\end{proof}
\begin{definicao}
	Dado um conjunto \(A\), o único \(U\) (pelo axioma da extensão) tal que \( \forall D ( \exists B (D\in B \land B\in A) \iff D \in U) \) é denotado por \[ U =  \bigcup A = \bigcup_{X\in A} X  \]
\end{definicao}
\begin{prop}
	\[ \bigcup \varnothing = \varnothing \]
\end{prop}
\begin{proof}
	De fato, \( D\in \bigcup \varnothing \iff \exists B \: (D\in B \land B\in \varnothing  \), mas isso implicaria em \( B\in \varnothing\), condtradição. Logo não existe \(D\) tal que \(D\in \bigcup \varnothing\). Logo \(\bigcup \varnothing = \varnothing\).
\end{proof}
\begin{prop}
	Se \(A\) é um conjunto, temos:
	\[ \bigcup \{ A \}  = A \]
\end{prop}
\begin{proof}
	Temos \[ D\in   \bigcup \{ A \}  \iff \exists B \: ( D\in B \land B\in \{ A\} ) \iff \exists B \: (D\in B \land B = A \iff D\in A . \]

	Logo \( \bigcup \{ A\} = A\).
\end{proof}
\begin{definicao}
	Sejam \(A,B\) conjuntos quaisquer, definimos a notação:
	\[ \bigcup \{ A,B\} = A\cup B \]
\end{definicao}
\begin{prop}
	Dados \(A,B\) conjuntos quaiser, temos
	\[ \forall D \: : \: (D\in A\cup B \iff D\in A \lor D \in B )\]
\end{prop}
\begin{proof}
	De fato, dado \(D\) qualquer, temos: 
	\begin{align*}
		D \in A\cup B &\iff D \in  \bigcup \{ A,B\} \\~\\
		&\iff \exists C \: ( D\in C \land C\in \{ A, B\} )\\~\\
		&\iff \exists C \: ( D\in C \land C\in \{ A, B\} )\\~\\
		&\iff \exists C \: ( D\in C \land ( C = A \lor C = B) )\\~\\
		&\iff \exists C \: ( ( D\in C \land  C = A)  \lor (D\in C \land C = B ) )\\~\\
		&\iff \exists C \: ( ( D\in C \land  C = A) )   \lor \exists C \:  (D\in C \land C = B)  \\~\\
		&\iff D\in A   \lor D\in B \\~\\
	\end{align*}
\end{proof}
\begin{prop}
	Sejam \(A,B,C\) conjuntos quaisquer, temos:
	\begin{enumerate}
...
\begin{prop}
	Dado um conjunto \(A\neq \emptyset\), existe um conjunto \(V\) tal que \(D \in V \iff \forall B ( B\in A \implies D\in B) \).
\end{prop}
\begin{proof}
	De fato, como \(A\neq \emptyset\), existe \(C \in A\). Tomando \( V = \{ D\in C \: | \: \forall B( B\in A \implies D\in B)\} \) pelo axioma da especifidade, temos 
	\begin{align*}
		D\in V &\iff D\in C \land \forall B ( B\in A \implies D \in B )\\~\\
		&\iff \forall B ( D\in C )  \land \forall B ( B\in A \implies D \in B )\\~\\
		&\iff \forall B (  D\in C )  \land \forall B ( B\in A \implies D \in B )\\~\\
	\end{align*}
\end{proof}
...
%\subsection{ Conjuntos Finitos, Enumeráveis e Não-Enumeráveis
%   \subsubsection{ Números naturais
%   \subsubsection{Boa ordenação e o Segundo Princípio de Indução
%   \subsubsection{Conjuntos finitos e infinitos
%   \subsubsection{Conjuntos enumeráveis
%   \subsubsection{Conjuntos não-enumeráveis
...
\subsection{ Funções Reais de n Variáveis}
   ...
   \begin{proof}
	   \begin{align*}
		   \lim_{x \to a} f(x) &= \lim_{v \to 0} f(a+v) \\~\\ 	 %provar
...
\[ f(a+v) - f(a) = \sum_{i=1}^n A_i \cdot \alpha_i  + \rho(v)\cdot |v|, \text{ onde } \lim_{v \to 0} \rho(v) = 0.\]
   ...
   \begin{prop}
	   Seja \(f : U \subset \mathbb{R}^{n+1} \to \mathbb{R} \) uma função de classe \(C^{k} \). Seja \((x_0,y_0)  \in U \) com \(x_0 \in \mathbb{R}^n \) e \(y_0  \in \mathbb{R}\) e \( \dfrac{\partial f}{\partial y}(x_0 ,y_0 ) \neq 0 \).  Então existem abertos \( V\subset \mathbb{R}^n \)  , \( W \subset \mathbb{R}\) com \( (x_0 , y_0 ) \in V\times W \)  e uma função \( \phi: V \to W \) com \( \forall (x,y) \in \) %carro
   \end{prop}
   \begin{proof}
...
\end{prop}
\begin{proof}
	Seja $T\in GL(\mathbb{R}^n$. Logo $T$ é  invertível (bijetiva). Tomando \( \delta = \dfrac{1}{|T^{-1}|} \).
\end{proof}
\begin{prop}
...
\begin{proof}
	Seja $S = \text{Graf}(f) = \{ (x,y,z) \in \mathbb{R}^3 | (x,y) \in U \land z = f(x,y) \}$. Tomando $\phi: U \to S\cap \mathbb{R}^3 = S$ com $\phi(x,y) = (x,y, f(x,y))$. Temos $\phi$ contínua com $(\phi)^{-1} = \pi_{|S}$ (linear, logo contínua), logo é um homeomorfismo. Além disso, $D\phi(x,y) = \begin{bmatrix} 1 & 0 \\ 0 & 1 \\ f_1(x,y) & f_2(x,y) \end{bmatrix}$ com $\rank(D\phi(x,y)) = 2$ para todo $(x,y) \in U$, logo é uma imersão.
\end{proof}
\begin{definicao}
...
\end{definicao}
\begin{prop}
	Se \( f: U \subset \mathbb{R}^3 \to \mathbb{R} \) é uma função diferenciável e \( a\in f(U) \) é um valor regular de \( f\), então \( f^{-1}(a) \) é uma superfície regular em \( \mathbb{R}^3 \).
\end{prop}
\begin{proof}
	Seja \( p = (p_0, p_1, p_3) \in f^{-1}(a) \). Como \(a\) é um valor regular de \(f\), temos que \( Df(p) = \nabla f(p) \) é sobrejetiva. Logo podemos supor, sem perda de generalidade, que \( \dfrac{\partial f}{\partial z}(p) \neq 0 \). Pelo Teorema da Função Implícita, existem abertos \( V\subset \mathbb{R}^2 , W \subset \mathbb{R}\) tal que \( p \in V\times W \subset U \) e uma função diferenciável \( \phi : V \to W \) tal que \( \forall (x,y,z) \in V\times W \: : \: f(x,y,z) = f(p) = a \iff z = \phi(x,y) \). Tomando \( h: V \to f^{-1}(a) \cap ( V\times W) \) dada por \( h(x,y) = (x,y, \phi(x,y) ) \). Temos \(h\) diferenciável, com \( \rank( Dh(q)) = 2 \) para todo \(q\in V\). Além disso, temos \( h^{-1} = \pi|_{f^{-1}(a)\cap (V\times W)} \), onde \( \pi \) é a projeção das primeiras duas coordenadas. De fato, se \( (x,y) \in V\), temos \( \pi( h(x,y)) = \pi( (x,y, \phi(x,y))) = (x,y) \). Se \( (x,y,z) \in f^{-1}(a) \cap (V\times W) \), temos  \( f(x,y,z) = a \iff z = \phi(x,y) \), pois \( (x,y,z) \in V\times W\). Logo \( h(\pi(x,y,z))) = h(x,y) = (x,y,\phi(x,y)) = (x,y,z) \).  Logo \( h\) parametriza a superfície em volta de \(p \) qualquer.
\end{proof}
