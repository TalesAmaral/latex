...
\end{proof}
\begin{obs}
	A função construída na proposição anterior é estritamente crescente. De fato, seja $Y = \left\{ n \in \mathbb{N} \: | \: \forall m \in \mathbb{N} \: : \:  m > n \implies f(m) > f(n)\right\}$. Temos $1\in X$, dado $m\in \mathbb{N}$ com $m > 1$, temos  $  m = m_0 +1$ com $m_0 \in \mathbb{N}$, daí  $ f(m) = \min( X- f\left(I_{m_0}\right))$ . Como $f(1) \in f\left(I_{m_0}\right)$, temos $f(m) \neq f(1)$. Como $f(m) \in X$ e $f(m) \neq f(1)$, temos  $f(1) = \min X  < f(m)$ para todo $m>1$. Se $n\in X$, temos $\forall m \in \mathbb{N}\: : \: m > n \implies f(m) > f(n)$. Dado $m\in \mathbb{N}$ com $m > n+1 > n$, temos $f(m) > f(n)$ e $f(n+1) > f(n)$. Como $I_{n+1} $. %carro
\end{obs}
\begin{prop}
...
\section{Exercícios}
\begin{exercicio}
	Se $X \subset \mathbb{N}$ é  infinito, então existe uma única bijeção $\phi: \mathbb{N} \to \mathbb{X}$ crescente.
\end{exercicio}
\begin{proof}
	Tomo subconjunto dos naturais é enumerável, existe uma bijeção $f : \mathbb{N} \to X$. Definindo $\phi: \mathbb{N} \to X$, dada por $\phi(1) = \min X = \min f\left(\mathbb{N}\right)$ 
\end{proof}
